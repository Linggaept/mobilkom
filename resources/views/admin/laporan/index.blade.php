...
<div class="page-header d-flex align-items-center justify-content-between">
    <div>
        <h4><i class="bi bi-clipboard2-data me-2"></i>Kelola Laporan</h4>
        <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li><li class="breadcrumb-item active">Laporan</li></ol></nav>
    </div>
</div>
        ...
        <form method="GET" class="row g-2" id="filterForm">
            ...
            <div class="col-md-2">
                <select name="status" class="form-select form-select-sm">
                    ...
                    @foreach(['menunggu_verifikasi'=>'Menunggu Verifikasi','diverifikasi'=>'Diverifikasi','sedang_proses'=>'Sedang Proses','selesai'=>'Selesai','ditolak'=>'Ditolak'] as $k=>$v)
                    <option value="{{ $k }}" {{ request('status')==$k?'selected':'' }}>{{ $v }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-3"><input type="text" name="search" class="form-control form-control-sm" placeholder="Cari nomor/nama..." value="{{ request('search') }}"></div>
            <div class="col-md-3 d-flex gap-1">
                <button type="submit" class="btn btn-primary btn-sm"><i class="bi bi-search me-1"></i>Cari</button>
                <a href="{{ route('admin.laporan.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x-lg"></i></a>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <div class="d-flex gap-1 justify-content-center">
                            <a href="{{ route('admin.laporan.show', $l->id) }}" class="btn btn-sm btn-outline-primary py-0 px-2" title="Detail"><i class="bi bi-eye"></i></a>
                            <a href="{{ route('admin.laporan.cetak', $l->id) }}" target="_blank" class="btn btn-sm btn-outline-secondary py-0 px-2" title="PDF"><i class="bi bi-file-pdf"></i></a>
                        </div>
                    </td>
                </tr>
                @empty
...
<form id="bulkPrintForm" action="{{ route('admin.laporan.cetakBulk') }}" method="POST" target="_blank" class="d-none">
    @csrf
    <div id="bulkIds"></div>
</form>
@endsection
